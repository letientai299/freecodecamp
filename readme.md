...
test, but I really hate:

- Unclear spec that force us to guess what the author means.
- Hidden tests that make it extremely frustrated to pass them. In real world, we
  have [black box testing](https://en.wikipedia.org/wiki/Black-box_testing), in
  which the code is hidden, but the tests are visible. This is the opposite.
  Yes, I could find the source for the tests, but that doesn't change my feeling
  about hidden tests.

As a result of such bad feelings, I have to do my best to get rid of those
projects as quicly as possible, including writing dummy tests to pass the FCC
tests. Sorry.
...
| [Personal Library][fcc_qa_lib]                   | [./qa/lib](./qa/lib)               | [??](https://fcc-box.herokuapp.com/qa/)                  |
